    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.galleries.update", [$gallery->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label>{{ trans('cruds.gallery.fields.title') }}</label>
                <select class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" name="title" id="title">
                    ...
                    @foreach(App\Models\Gallery::TITLE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('title', $gallery->title) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('title'))
