...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.bookinSeat.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.bookin-seats.update", [$bookinSeat->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('hall') ? 'is-invalid' : '' }}" name="hall_id" id="hall_id">
                    @foreach($halls as $id => $entry)
                        <option value="{{ $id }}" {{ (old('hall_id') ? old('hall_id') : $bookinSeat->hall->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('hall'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id">
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $bookinSeat->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('seat_code') ? 'is-invalid' : '' }}" type="text" name="seat_code" id="seat_code" value="{{ old('seat_code', $bookinSeat->seat_code) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('row') ? 'is-invalid' : '' }}" type="text" name="row" id="row" value="{{ old('row', $bookinSeat->row) }}">
                @if($errors->has('row'))
                    <div class="invalid-feedback">
                        {{ $errors->first('row') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('column') ? 'is-invalid' : '' }}" type="text" name="column" id="column" value="{{ old('column', $bookinSeat->column) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('price') ? 'is-invalid' : '' }}" type="text" name="price" id="price" value="{{ old('price', $bookinSeat->price) }}">
                @if($errors->has('price'))
                    <div class="invalid-feedback">
                        {{ $errors->first('price') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\BookinSeat::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $bookinSeat->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
